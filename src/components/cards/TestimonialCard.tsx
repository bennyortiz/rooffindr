...
export function TestimonialCard({
  ...
  return (
    <div
      className={cn('bg-white/10 p-6 rounded-lg backdrop-blur-sm h-full flex flex-col', className)}
    >
      <QuoteIcon className={`h-6 w-6 text-primary mb-4 flex-shrink-0`} />
      <p className={cn('mb-4 text-base flex-grow', textColor)}>{testimonial.quote}</p>
      <div className="flex items-center mt-auto">
        <Avatar className="mr-3 border-2 border-white/20 flex-shrink-0">
          {testimonial.image ? (
